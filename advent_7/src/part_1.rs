...
fn sum_directories(mut nodes: Vec<Directory>) -> usize {
    let mut under_size: Vec<usize> = Vec::new();
    let mut node = 0;
    ...
    loop {
        ...
        if unchecked > 0 { //this is the unchecked child
            ...
            if sum <= 100000 {
                under_size.push(node); //under the 100000 marker add to vector 
                result += sum;
            }
